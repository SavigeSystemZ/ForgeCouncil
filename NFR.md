...
| SEC-13 | When `FC_ARTIFACT_ROOT` is set, log files are written only under `{root}/{run_id}/{step_id}/` with path containment checks; treat the tree as **sensitive** (chmod, backups, retention). |
